...
const $ = (scope: WeightRule['scope'], n: number | undefined, weight: Weight, shutokuOnly: boolean = false): WeightRule => ({ scope, n, weight, shutokuOnly });

export const apply = (weighted: WeightedUnit[], rules: readonly WeightRule[]) => {
  for (const { scope, n, weight, shutokuOnly } of rules) {
    const targets = weighted
      .filter(isSpecificWeightedUnit)
      .filter(shutokuOnly ? (w => scope(w.report.course.code) && !['不可', '欠席', '未履修'].includes(w.report.grade)) : (w => scope(w.report.course.code)))
...
export const requiredCourseAllOne = (req: cr.CourseRequirement) => [
  $(isSubcourseOf(...cr.collectAllChoices(req).map(r => r.code)), void 0, 1),
];

export const none: WeightRuleGenerator = () => [];
export const 地域文化研究: WeightRuleGenerator = () => [
  $(isSubcourseOf('FC1'), 4, 1.5),
  $(isSubcourseOf('FC2'), 4, 1.5),
];
export const 総合社会科学 = () => [
  $(isSubcourseOf('FC6'), 8, 2),
  $(isSubcourseOf('GCA2', 'GCA5', 'GCB1', 'GCB5', 'GCC1', 'GCC2', 'GCC3', 'GCC4', 'GCF3'), 4, 2),
];
export const 数理自然科学: WeightRuleGenerator = karui => karui.startsWith('NS') ? [
  $(isSubcourseOf('FC3', 'FC8'), 8, 1.5),
] : [];
...
export const 国際日本研究PEAK後期: WeightRuleGenerator = () => [
  $(isSubcourseOf('FC11'), void 0, 2),
];
export const 初ゼミ０: WeightRuleGenerator = () => [
  $(isSubcourseOf('FC510'), 2, 0),
];
export const 計数: WeightRuleGenerator = () => [
  $(isSubcourseOf('FC871', 'FC873', 'FC874', 'FC875', 'FC876', 'FC881', 'FC882', 'FC883', 'FC884'), void 0, 2),
  $(isSubcourseOf('FC3', 'FC885', 'FC886', 'FC887', 'FC888', 'FC891', 'FC892', 'FC893'), void 0, 1.5),
];
export const 地球惑星物理学: WeightRuleGenerator = () => [
  $(isSubcourseOf('FC871', 'FC873', 'FC874', 'FC875', 'FC876', 'FC881', 'FC882', 'FC883', 'FC884', 'FC885', 'FC886'), 14, 2),
];
export const 生物化学: WeightRuleGenerator = karui => karui.startsWith('NS') ? [
  $(isSubcourseOf('GCE', 'GCF'), 6, 2, true),
] : [];
export const 応用生物学: WeightRuleGenerator = () => [
  $(isSubcourseOf('FC'), 16, 5),
  $(isSubcourseOf('GC'), 4, 5),
];
export const 森林生物科学: WeightRuleGenerator = () => [
  $(isSubcourseOf('FC', 'GC'), 20, 5),
];
export const 水圏生物科学: WeightRuleGenerator = karui => karui.startsWith('NS') ? [
  $(isSubcourseOf('FC8'), 6, 5),
  $(isSubcourseOf('GCA', 'GCB', 'GCC', 'GCD', 'GCE', 'GCF'), 6, 5, true),
] : [];
export const 生物素材化学: WeightRuleGenerator = karui => karui.startsWith('NS') ? [
  $(isSubcourseOf('FC8'), 10, 5),
  $(isSubcourseOf('GCD', 'GCE', 'GCF'), 4, 5, true),
] : [];
export const 農業資源経済学: WeightRuleGenerator = karui =>
  karui.startsWith('HSS') ? [ $(isSubcourseOf('FC63'), 4, 2, true) ] :
  karui.startsWith('NS') ? [ $(isSubcourseOf('GCC32', 'GCC41', 'GCC42'), 6, 2, true) ] :
  unreachable('文科と理科しかないことを想定して書いている');
export const フィールド科学: WeightRuleGenerator = () => [
  $(isSubcourseOf('FC'), 10, 5),
];
